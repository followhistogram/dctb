...
-- Allow public read access to active references
CREATE POLICY "Allow public read access to active references" 
ON references FOR SELECT 
USING (is_active = true);

-- Allow authenticated users full access (for admin)
CREATE POLICY "Allow authenticated users full access to references" 
ON references FOR ALL 
USING (auth.role() = 'authenticated');

-- Insert some sample data
INSERT INTO references (name, position, text, rating, display_order) VALUES
