...
import { updateHeroContent } from "@/lib/hero-actions"
...
export function HeroForm({ content }: { content: HeroContent }) {
  const [state, formAction] = useFormState(updateHeroContent, initialState)
    ...
    <form action={handleSubmit}>
      ...
      <div className="mt-6 flex justify-end">
        <Button type="submit" disabled={isPending}>
          <Save className="mr-2 h-4 w-4" /> {isPending ? "Ukládám..." : "Uložit změny"}
        </Button>
      </div>
    </form>
  )
